...
import datetime
import calendar
from general_stat.views import get_user_token_cookie
...
def get_daily_stat(site_id, person_id, start, end, token):
...
def show_daily_page(request):
    ...
    sites = general_stat.views.get_sites(token)
    persons = get_persons(token)
    ...
    daily_stat = get_daily_stat(site_id, person_id, f_start,f_end, token)
